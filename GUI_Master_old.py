...
#
lbl = tk.Label(root, text="Road Pathole detection", font=('times', 35,' bold '), width=65,height=1,bg="Red3",fg="white")
lbl.place(x=0, y=0)
...
def test_model_proc(fn):
    ...
    if fn!="":
        ...
        return A
    img = open(fn)    
    myfile = open("E:/road pathole/road_pathole/training/1/1.txt")
    if img == myfile:
          print(myfile)
...
def openimage():
    ...
    global fn
    print(fn)
    fileName = askopenfilename(initialdir='E:/road pathole/road_pathole', title='Select image for Aanalysis ',
                               filetypes=[("all files", "*.*")])
    IMAGE_SIZE=200
    imgpath = fileName
    fn = fileName
